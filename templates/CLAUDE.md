...
<!-- WHY: In multi-agent setups, each agent needs to know its scope BEFORE starting work.
Without this, agents overlap, conflict, or miss responsibilities. -->

If you are working on this project as an implementation agent, read your instruction file first:
- **Backend agent:** Read `.agent-instructions/backend-agent.md` before starting any work.
- **Frontend agent:** Read `.agent-instructions/frontend-agent.md` before starting any work.

Update your state file (`.agent-state/backend-agent.md` or `.agent-state/frontend-agent.md`)
...
document it in your `.agent-state/*.md` file under "Contract Deviations" -- do not modify
...
results in your state file. Do not mark a task complete based on "it should work."
